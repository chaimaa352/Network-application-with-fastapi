...
def add_pagination_links(base_url: str, endpoint: str, page: int, limit: int, total: int, **params) -> dict:
...
async def get_comments(
    ...
    comments, total = await comment_service.get_comments(page, limit, sort_by, sort_order.value, filters)
    ...
    return PaginatedResponse(
        ...
        _links=add_pagination_links(
            base_url, endpoint, page, limit, total, sort_by=sort_by, sort_order=sort_order, post=post, user=user
        ),
    )
...
async def get_comments_by_post(
    ...
    return PaginatedResponse(
        ...
        limit=limit,
        _links=add_pagination_links(
            base_url, f"/api/v1/comments/post/{post_id}", page, limit, total, sort_by=sort_by, sort_order=sort_order
        ),
    )
...
async def get_comments_by_user(
    ...
    return PaginatedResponse(
        ...
        limit=limit,
        _links=add_pagination_links(
            base_url, f"/api/v1/comments/user/{user_id}", page, limit, total, sort_by=sort_by, sort_order=sort_order
        ),
    )


@router.post("", response_model=Comment, status_code=201, summary="Create comment", description="Create a new comment")
async def create_comment(request: Request, comment_data: CommentCreate, accept_language: Optional[str] = Header("en")):
    """Create new comment"""
...
@router.delete(
    "/{comment_id}", response_model=DeleteResponse, summary="Delete comment", description="Delete comment by ID"
)
async def delete_comment(comment_id: str = Path(...), accept_language: Optional[str] = Header("en")):
